...
def build_transform_aug(cfg, is_train=True, choices=None):
    """
    Build transformation function.
    * Transform: perform some manipulation of the data and make it suitable for training.
    Augmentation: a common technique that can improve the performance of models
    ...
    Args:
        ...
        choices (list, optional): list of strings which will overwrite
            cfg.inputs.TRANSFORMS if given. Default is None.
    """
    if cfg.inputs.no_transform:
        print("Note: no transform is applied!")
        return None

    if choices is None:
        choices = cfg.inputs.transforms
    ...
    target_size = f"{cfg.inputs.size[0]}x{cfg.inputs.size[1]}"
    #target_size = cfg.inputs.size

    normalize = Normalize(mean=cfg.inputs.pixel_mean, std=cfg.inputs.pixel_std)
...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    tfm_train = [] #ToPILImage()

    interp_mode = INTERPOLATION_MODES[cfg.inputs.interpolation]
    input_size = cfg.inputs.size
    ...
    if "randaugment" in choices:
        n_ = cfg.inputs.randaugment_n
        m_ = cfg.inputs.randaugment_m
    ...
    if "colorjitter" in choices:
        b_ = cfg.inputs.colorjitter_b
        c_ = cfg.inputs.colorjitter_c
        s_ = cfg.inputs.colorjitter_s
        h_ = cfg.inputs.colorjitter_h
        print(
    ...
    if "randomgrayscale" in choices:
        ...
        tfm_train += [RandomGrayscale(p=cfg.inputs.rgs_p)]

    if "gaussian_blur" in choices:
        print(f"+ gaussian blur (kernel={cfg.inputs.gb_k})")
        gb_k, gb_p = cfg.inputs.gb_k, cfg.inputs.gb_p
    ...
    if "cutout" in choices:
        cutout_n = cfg.inputs.cutout_n
        cutout_len = cfg.inputs.cutout_len
        print(f"+ cutout (n_holes={cutout_n}, length={cutout_len})")
        tfm_train += [Cutout(cutout_n, cutout_len)]

    if "normalize" in choices:
        print(
            f"+ normalization (mean={cfg.inputs.pixel_mean}, std={cfg.inputs.pixel_std})"
        )
        tfm_train += [normalize]

    if "gaussian_noise" in choices:
        print(
            f"+ gaussian noise (mean={cfg.inputs.gn_mean}, std={cfg.inputs.gn_std})"
        )
        tfm_train += [GaussianNoise(cfg.inputs.gn_mean, cfg.inputs.gn_std)]
